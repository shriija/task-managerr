...
export const useBoardStore = create((set, get) => ({
  ...
  fetchDeletedListsAndCards: async (boardId) => {
    try {
      const listsRes = await axios.get(`${API}/list-api/trash/deleted/${boardId}`, { withCredentials: true })
      const cardsRes = await axios.get(`${API}/card-api/trash/deleted/${boardId}`, { withCredentials: true })
      set({ deletedLists: listsRes.data.payload, deletedCards: cardsRes.data.payload })
    } catch (err) { console.error(err) }
  },
  ...
  reset: () => {
    ...
    set({ board: null, lists: [], deletedLists: [], deletedCards: [], loading: false, error: null, onlineUsers: [] })
  }
